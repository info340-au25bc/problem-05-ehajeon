...
lostGames.forEach(function(game) {
  ...
  if (game.opponent == "UW") {
    console.log("UW at " + game.opponent + ", " + game.home_score + " to " + game.opponent_score);
  }
});
...
function mostYardsPassing(gameOne, gameTwo) {
  if (gameTwo.passing_yards > gameOne.passing_yards || gameOne.passing_yards === undefined) {
    return gameTwo;
  }
  return gameOne;
}
...
function makeCombinedFilter(functionOne, functionTwo) {
  function combinedFilter(game) {
    return functionOne(game) && functionTwo(game);
  }
